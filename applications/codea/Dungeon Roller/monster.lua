...
function Monster:moveToHero()
    local line = (self.position-self.hero.position):normalizeInPlace(2)
    
    self.position:add(line)
end

function Monster:moveAwayFromHero(amount)
    local line = (self.hero.position-self.position):normalizeInPlace(amount)
...
function Monster:draw()
    ...
    if self.dead then
        ...
        self.position:add(self.knockVel)
