...
extern "C" {
    ...
    unref unrefJoin = NULL;
    EXPORT void setUnrefJoint(unref func) {
        unrefJoin = func;
    }
	...
	EXPORT b2World* b2World_new() {
        ...
        world->SetDestructionListener(new MyDestructionListener());
		return world;
	}
    ...
    EXPORT void b2World_setAutoClearForces(b2World* self, bool autoClearForces) {
	...
	EXPORT void b2Body_gc(b2World* _world, b2Body* self) {
	...
	EXPORT void b2Body_applyForce(b2Body* self, b2Vec2* force, b2Vec2* point) {
		self->ApplyForce(*force, *point, true);
	}
	
	EXPORT void b2Body_applyForceToCenter(b2Body* self, b2Vec2* force) {
	...
	EXPORT void b2Joint_gc(b2World* world, b2Joint* self) {
